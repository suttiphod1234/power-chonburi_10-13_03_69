/**
 * Google Apps Script for Power BI T2 Quiz
...
function doPost(e) {
  ...
  if (!sheet) {
    ...
    // Set Headers for T2 Quiz (10 Questions)
    var headers = [
      "Timestamp", "ชื่อ-นามสกุล", "อีเมล", "เบอร์โทรศัพท์", "ตำแหน่งงานปัจจุบัน", 
      "คะแนน T2 (เต็ม 10)", "ข้อ 1", "ข้อ 2", "ข้อ 3", "ข้อ 4", "ข้อ 5", 
  ...
  try {
    ...
    if (data.email) {
      sendScoreEmailT2(data.email, data.name, data.score);
    }
...
function sendScoreEmailT2(email, name, score) {
  var subject = "ผลคะแนนแบบทดสอบ T2 - " + name;
  var body = "สวัสดีครับคุณ " + name + ",\n\n" +
             "คุณได้ทำแบบทดสอบความรู้ Power BI Visuals (T2) เรียบร้อยแล้ว\n" +
